...
{
    let rcmage = cmage.oriented(CGImagePropertyOrientation.right)
    let context:CIContext = CIContext.init(options: nil)
    let cgImage:CGImage = context.createCGImage(rcmage, from: rcmage.extent)!
    let image:UIImage = UIImage.init(cgImage: cgImage)
//    let image: UIImage = UIImage(cgImage: cgImage, scale: 1, orientation: .leftMirrored)
    return image
}
